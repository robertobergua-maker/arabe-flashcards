  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50 to-emerald-50 flex items-center justify-center p-4 relative overflow-hidden">
        ...
        <div className="bg-white/80 backdrop-blur-md p-8 md:p-12 rounded-3xl shadow-2xl text-center max-w-md w-full z-10 border border-white relative">
            <div className="absolute -top-4 -right-4 bg-yellow-400 text-yellow-900 text-xs font-bold px-3 py-1 rounded-full rotate-12 shadow-md animate-pulse flex items-center gap-1 notranslate" translate="no">
...
export default function App() {
  ...
  // ⚠️ SEGURIDAD: La contraseña está en el código fuente. Para producción, muévela a una variable de entorno (VITE_ADMIN_PASS o similar) o valídala en backend.
  const ADMIN_PASS = import.meta.env?.VITE_ADMIN_PASS || "1234";
  const handleAdminToggle = () => { if (isAdminMode) setIsAdminMode(false); else { const p = prompt("🔒 Contraseña:"); if (p === ADMIN_PASS) setIsAdminMode(true); } };
...
function GamesHub({ onClose, cards, showDiacritics }) {
  ...
  if (activeGame === 'idafa') return <IdafaGame onBack={() => setActiveGame('menu')} cards={cards} onClose={onClose} showDiacritics={showDiacritics} />;
    ...
    <div className="fixed inset-0 bg-black/90 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col relative">
        ...
        <div className="p-8 bg-slate-100 grid grid-cols-1 md:grid-cols-2 gap-6 overflow-y-auto max-h-[70vh]">
          ...
          <button onClick={() => setActiveGame('listening')} className="bg-white p-6 rounded-2xl shadow-md hover:shadow-xl transition-all text-left border border-slate-200 group">
          ...
          <button onClick={() => setActiveGame('idafa')} className="bg-white p-6 rounded-2xl shadow-md hover:shadow-xl transition-all text-left border border-slate-200 md:col-span-2 group">
            <div className="w-12 h-12 bg-teal-100 text-teal-600 rounded-xl flex items-center justify-center mb-4 group-hover:bg-teal-600 group-hover:text-white transition-colors"><span className="text-2xl font-arabic font-black">إضـ</span></div>
            <h3 className="text-xl font-bold text-slate-800 mb-2">La Idafa <span className="text-sm font-normal text-teal-600 ml-1">الإضافة</span></h3><p className="text-sm text-slate-500">Construye la estructura de posesión árabe en el orden correcto.</p>
          </button>
        </div>
      </div>
    </div>
...
function TrueFalseGame({ onBack, onClose, cards, showDiacritics }) {
  ...
  useEffect(() => { if (timer <= 0 && gameState === 'playing') { setGameOver(true); setGameState('timeout'); } }, [timer, gameState]);
  ...
  const processText = () => {
      try {
        ...
        const newCards = lines
          ...
          .filter(c => c.spanish && c.arabic); // ignorar líneas sin los campos obligatorios
        if (newCards.length === 0) { alert("No se encontraron líneas válidas. Formato esperado: Español | عربي | fonética"); return; }
        onImport(newCards);
...
// --- JUEGO: LA IDAFA الإضافة ---

// Elimina el artículo definido (ال) del inicio de una palabra árabe para formar el mudāf
const stripDefiniteArticle = (arabic) => {
  if (!arabic) return arabic;
  // Cubre variantes: الْ (con sukun) y ال (sin vocalización)
  return arabic.replace(/^(الْ|ال)/, '');
};

// Genera ejercicios de idafa emparejando tarjetas del vocabulario de Supabase
const buildIdafaExercises = (cards) => {
  const pool = shuffleArray(cards.filter(c => c.arabic && c.spanish));
  if (pool.length < 2) return [];
  const exercises = [];

  // Idafa básica: pares de 2 palabras
  const pairCount = Math.min(30, Math.floor(pool.length / 2));
  for (let i = 0; i < pairCount; i++) {
    const c1 = pool[i * 2];
    const c2 = pool[i * 2 + 1];
    const mudaf = stripDefiniteArticle(c1.arabic);
    exercises.push({
      spanish: `${c1.spanish} de ${c2.spanish}`,
      parts: [mudaf, c2.arabic],
      phonetic: '',
      level: 1,
    });
  }

  // Idafa triple: tríos de 3 palabras (si hay suficientes)
  if (pool.length >= 6) {
    const triplePool = shuffleArray([...pool]);
    const tripleCount = Math.min(10, Math.floor(triplePool.length / 3));
    for (let i = 0; i < tripleCount; i++) {
      const c1 = triplePool[i * 3];
      const c2 = triplePool[i * 3 + 1];
      const c3 = triplePool[i * 3 + 2];
      exercises.push({
        spanish: `${c1.spanish} de ${c2.spanish} de ${c3.spanish}`,
        parts: [stripDefiniteArticle(c1.arabic), stripDefiniteArticle(c2.arabic), c3.arabic],
        phonetic: '',
        level: 2,
      });
    }
...
function IdafaGame({ onBack, onClose, cards, showDiacritics }) {
  const [queue] = useState(() => buildIdafaExercises(cards));
  const [currentIndex, setCurrentIndex] = useState(0);
  const [tiles, setTiles] = useState([]);
  const [selected, setSelected] = useState([]);
  const [feedback, setFeedback] = useState(null); // null | 'correct' | 'wrong'
  const [score, setScore] = useState(0);
  const [highScore, setHighScore] = useState(() => safeGetStorage('idafa_highscore', 0));
  const [levelFilter, setLevelFilter] = useState(0); // 0=todos, 1=básica, 2=triple
  const [showFilter, setShowFilter] = useState(false);

  const filteredQueue = useMemo(() =>
    levelFilter === 0 ? queue : queue.filter(e => e.level === levelFilter),
    [queue, levelFilter]
  );

  const current = filteredQueue.length > 0 ? filteredQueue[currentIndex % filteredQueue.length] : null;

  useEffect(() => {
    if (current) {
      setTiles(shuffleArray(current.parts.map((p, i) => ({ id: i, text: p }))));
      setSelected([]);
      setFeedback(null);
    }
  }, [currentIndex, levelFilter]);
  ...
  const handleTileClick = (tile) => {
    ...
    if (newSelected.length === current.parts.length) {
      const isCorrect = newSelected.every((s, i) => s.text === current.parts[i]);
      if (isCorrect) {
        playSmartAudio(current.parts.join(' '));
        const newScore = score + 1;
        setScore(newScore);
        if (newScore > highScore) {
          setHighScore(newScore);
          localStorage.setItem('idafa_highscore', newScore.toString());
        }
        setFeedback('correct');
        setTimeout(() => setCurrentIndex(i => i + 1), 1800);
      } else {
        setFeedback('wrong');
        setScore(0);
        setTimeout(() => { setSelected([]); setFeedback(null); }, 1500);
      }
    }
  };

  const display = (text) => showDiacritics ? text : removeArabicDiacritics(text);

  if (!current) return (
    <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-8 text-white text-center flex-col gap-4">
      <span className="text-5xl">📚</span>
      <p className="text-xl font-bold">
        {queue.length === 0
          ? "Necesitas al menos 2 tarjetas en la base de datos para jugar a la Idafa."
          : "No hay ejercicios para este nivel con el vocabulario actual."}
      </p>
      <button onClick={onBack} className="mt-4 px-6 py-3 bg-teal-600 hover:bg-teal-500 rounded-xl font-bold transition-colors">Volver al menú</button>
    </div>
  );

  const isTriple = current.parts.length === 3;
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col">
        ...
        <div className="bg-teal-700 p-4 text-white flex justify-between items-center">
          <div className="flex items-center gap-2">
            <button onClick={onBack} className="hover:bg-teal-600 p-1 rounded mr-1"><ArrowLeft className="w-5 h-5"/></button>
            <h2 className="font-bold text-lg">La Idafa</h2>
            <span className="text-teal-300 font-arabic text-lg">الإضافة</span>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={() => setShowFilter(v => !v)}
              className="text-xs font-bold bg-teal-600/60 hover:bg-teal-600 px-3 py-1.5 rounded-lg transition-colors">
              {levelFilter === 0 ? 'Todos' : levelFilter === 1 ? 'Básica' : 'Triple'}
            </button>
            <button onClick={onClose} className="hover:bg-teal-600 p-1 rounded"><X className="w-5 h-5"/></button>
          </div>
        </div>

        {/* Filtro de nivel */}
        {showFilter && (
          <div className="bg-teal-900 px-4 py-3 flex gap-2 justify-center flex-wrap">
            {[{v:0,label:'Todos'},{v:1,label:'Idafa básica (2 palabras)'},{v:2,label:'Triple idafa (3 palabras)'}].map(({v,label}) => (
              <button key={v}
                onClick={() => { setLevelFilter(v); setShowFilter(false); setCurrentIndex(0); }}
                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-colors ${levelFilter===v ? 'bg-white text-teal-900' : 'text-white/70 hover:bg-teal-700'}`}>
                {label}
              </button>
            ))}
          </div>
        ...
        {/* Score + Nivel */}
        <div className="flex justify-between items-center px-6 py-3 bg-teal-50 border-b border-teal-100 shrink-0">
          <div className="flex flex-col items-center">
            <span className="text-xs font-bold text-teal-400 uppercase">Racha</span>
            <span className="text-xl font-black text-teal-700">{score}</span>
          </div>
          <span className={`text-[11px] font-black px-3 py-1 rounded-full ${isTriple ? 'bg-amber-100 text-amber-700' : 'bg-teal-100 text-teal-700'}`}>
            {isTriple ? '★★ Triple' : '★ Básica'}
          </span>
          <div className="flex flex-col items-center">
            <span className="text-xs font-bold text-amber-500 uppercase flex items-center gap-1"><Trophy className="w-3 h-3"/> Récord</span>
            <span className="text-xl font-black text-amber-600">{highScore}</span>
          </div>
        </div>

        {/* Regla gramatical */}
        <div className="mx-5 mt-4 bg-teal-50 border border-teal-200 rounded-xl px-4 py-2.5 flex items-start gap-3">
          <span className="text-lg mt-0.5">📐</span>
          <p className="text-xs text-teal-800 leading-relaxed">
            <span className="font-black">Mudāf</span> (sin artículo)
            {isTriple && <> + <span className="font-black">Mudāf</span> intermedio (sin artículo)</>}
            {' '}+ <span className="font-black">Mudāf ilayhi</span> (último, puede llevar artículo)
          </p>
        </div>

        {/* Pregunta */}
        <div className="px-5 pt-4 pb-2 text-center">
          <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">¿Cómo se dice?</span>
          <h3 className="text-2xl font-black text-slate-800 mt-1">{current.spanish}</h3>
        </div>

        {/* Zona de montaje */}
        <div className="mx-5 mb-3 min-h-[68px] border-2 border-dashed rounded-xl flex items-center justify-center gap-2 p-3
          bg-slate-50 border-slate-200" dir="rtl">
          {selected.length === 0
            ? <span className="text-sm text-slate-400 italic">Pulsa las piezas en orden →</span>
            : selected.map(tile => (
              <button key={tile.id} onClick={() => !feedback && handleTileClick(tile)}
                className={`font-arabic text-2xl px-4 py-2 rounded-xl font-bold border-2 transition-all
                  ${feedback === 'correct'
                    ? 'bg-green-100 border-green-400 text-green-900 scale-105'
                    : feedback === 'wrong'
                    ? 'bg-red-100 border-red-400 text-red-900'
                    : 'bg-white border-teal-400 text-teal-900 shadow hover:bg-red-50 hover:border-red-300'}`}>
                {display(tile.text)}
              </button>
            ))
          }
        ...
        {/* Tiles disponibles */}
        <div className="px-5 pb-3 flex flex-wrap gap-3 justify-center" dir="rtl">
          {tiles.map(tile => {
            const isUsed = !!selected.find(s => s.id === tile.id);
            return (
              <button key={tile.id} onClick={() => handleTileClick(tile)} disabled={!!feedback}
                className={`font-arabic text-2xl px-5 py-2.5 rounded-xl font-bold border-2 transition-all active:scale-95
                  ${isUsed
                    ? 'opacity-25 bg-slate-100 border-slate-200 text-slate-400 cursor-not-allowed'
                    : 'bg-white border-slate-300 text-slate-800 hover:border-teal-500 hover:bg-teal-50 shadow-sm hover:shadow-md cursor-pointer'}`}>
                {display(tile.text)}
              </button>
            );
          })}
        ...
        {/* Feedback */}
        {feedback && (
          <div className={`mx-5 mb-4 p-3 rounded-xl text-center text-sm font-bold flex flex-col items-center gap-1
            ${feedback === 'correct' ? 'bg-green-50 border border-green-200 text-green-800' : 'bg-red-50 border border-red-200 text-red-800'}`}>
            {feedback === 'correct' ? (
              <>
                <span className="flex items-center gap-1.5"><CheckCircle className="w-4 h-4"/> ¡Correcto!</span>
                <span className="font-mono font-normal text-xs text-green-600">{current.phonetic}</span>
              </>
            ) : (
              <>
                <span className="flex items-center gap-1.5"><X className="w-4 h-4"/> El orden correcto es:</span>
                <span className="font-arabic text-xl text-red-900 font-bold" dir="rtl">
                  {current.parts.map(p => display(p)).join(' ')}
                </span>
                <span className="font-mono font-normal text-xs text-red-500">{current.phonetic}</span>
              </>
            )}
          </div>
        )}
