...
import { 
  ...
  Image as ImageIcon, MoreHorizontal, Menu, Wand2, Loader2,
    ...
    <div className="min-h-screen bg-slate-100 text-slate-800 font-sans flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto bg-slate-100 p-4 md:p-8">
          <div className="max-w-6xl mx-auto">
            ...
            {loading ? (
               <div className="flex flex-col items-center justify-center py-20 text-slate-400 gap-3"><Loader2 className="w-8 h-8 animate-spin text-emerald-600" /><span className="font-medium animate-pulse">Cargando datos...</span></div>
...
function SmartImportModal({ onClose, onImport }) {
  ...
  const process = () => {
      try {
          ...
          const newCards = lines.map(line => { const parts = line.split('|'); return { category: 'Importado', spanish: parts[0]?.trim(), arabic: parts[1]?.trim(), phonetic: parts[2]?.trim() || '' }; });
          onImport(newCards);
  ...
  return (
    <div className="fixed inset-0 bg-black/90 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col relative">
        <div className="bg-slate-800 p-6 text-white flex justify-between items-center"><div className="flex items-center gap-3"><Play className="w-8 h-8 text-yellow-400" /><h2 className="font-bold text-2xl">Arcade</h2></div><button onClick={onClose} className="hover:bg-slate-700 p-2 rounded-full transition"><X className="w-6 h-6" /></button></div>
        <div className="p-8 bg-slate-100 grid grid-cols-1 md:grid-cols-2 gap-6 overflow-y-auto max-h-[70vh]">
          <button onClick={() => setActiveGame('quiz')} className="bg-white p-6 rounded-2xl shadow-md hover:shadow-xl transition-all text-left border border-slate-200"><div className="w-12 h-12 bg-indigo-100 text-indigo-600 rounded-xl flex items-center justify-center mb-4 text-2xl">🧠</div><h3 className="text-xl font-bold text-slate-800 mb-2">Quiz Express</h3><p className="text-sm text-slate-500">Elige la traducción correcta.</p></button>
...
function ListeningGame({ onBack, onClose, cards, showDiacritics }) {
  ...
  const startNewRound = () => { if (cards.length < 4) return; const correctCard = cards[Math.floor(Math.random() * cards.length)]; const targetType = getCardType(correctCard); let candidates = cards.filter(c => c.id !== correctCard.id && getCardType(c) === targetType); if (candidates.length < 3) candidates = cards.filter(c => c.id !== correctCard.id); const distractors = shuffleArray(candidates).slice(0, 3); setRound({ card: correctCard, options: shuffleArray([correctCard, ...distractors]) }); setSelectedOption(null); setIsCorrect(null); setTimeout(() => playSmartAudio(correctCard.arabic), 500); };
  const handleOptionClick = (option) => { if (selectedOption) return; setSelectedOption(option); const correct = option.id === round.card.id; setIsCorrect(correct); if (correct) { const newScore = score + 1; setScore(newScore); if (newScore > highScore) { setHighScore(newScore); localStorage.setItem('listen_highscore', newScore.toString()); } setTimeout(startNewRound, 1500); } else { setScore(0); setTimeout(startNewRound, 2500); } };
  ...
  return (
    <div className="fixed inset-0 bg-black/90 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col relative">
        ...
        <div className="p-8 text-center bg-slate-50 flex flex-col items-center justify-center min-h-[180px]">{selectedOption ? ( <div className="animate-fade-in-up"><span className="text-xs font-bold text-slate-400 uppercase mb-2 block">Es...</span><h3 className="text-3xl font-black font-arabic text-slate-800 mb-2" dir="rtl">{showDiacritics ? round.card.arabic : removeArabicDiacritics(round.card.arabic)}</h3></div> ) : <button onClick={() => playSmartAudio(round.card.arabic)} className="w-24 h-24 rounded-full bg-cyan-100 text-cyan-600 flex items-center justify-center hover:scale-110 transition-all shadow-lg border-4 border-white"><Volume2 className="w-12 h-12" /></button>}</div>
...
function LetterGapGame({ onBack, onClose, cards, showDiacritics }) {
  ...
  const [isCorrect, setIsCorrect] = useState(null);
  useEffect(() => { startNewRound(); }, []);
  const startNewRound = () => { if (cards.length < 4) return; let candidate = null; for(let i=0; i<20; i++) { const randomCard = cards[Math.floor(Math.random() * cards.length)]; const clean = removeArabicDiacritics(randomCard.arabic); if (clean.length > 1 && getCardType(randomCard) === 'word') { candidate = randomCard; break; } } if (!candidate) candidate = cards[Math.floor(Math.random() * cards.length)]; const originalText = showDiacritics ? candidate.arabic : removeArabicDiacritics(candidate.arabic); const textArray = Array.from(originalText); const validIndices = textArray.map((char, index) => ARABIC_ALPHABET.includes(char) || ARABIC_ALPHABET.some(l => removeArabicDiacritics(char) === l) ? index : -1).filter(i => i !== -1); if (validIndices.length === 0) { setTimeout(startNewRound, 100); return; } const indexToHide = validIndices[Math.floor(Math.random() * validIndices.length)]; const correctLetter = textArray[indexToHide]; let distractors = []; while(distractors.length < 3) { const randomLetter = ARABIC_ALPHABET[Math.floor(Math.random() * ARABIC_ALPHABET.length)]; if (removeArabicDiacritics(randomLetter) !== removeArabicDiacritics(correctLetter)) distractors.push(randomLetter); } const maskedTextArray = [...textArray]; maskedTextArray[indexToHide] = "___"; setRound({ card: candidate, before: textArray.slice(0, indexToHide).join(""), after: textArray.slice(indexToHide + 1).join(""), correctOption: correctLetter, options: shuffleArray([correctLetter, ...distractors]) }); setSelectedOption(null); setIsCorrect(null); };
  ...
  return (
    <div className="fixed inset-0 bg-black/90 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col relative">
        ...
        <div className="p-8 text-center bg-slate-50 flex flex-col items-center justify-center min-h-[200px]"><span className="text-xs font-bold text-slate-400 uppercase mb-4 block">Completa: {round.card.spanish}</span><div className="flex items-center justify-center gap-1 text-4xl md:text-5xl font-black font-arabic text-slate-800" dir="rtl"><span>{round.before}</span><div className={`w-12 h-12 border-b-4 rounded-md flex items-center justify-center text-2xl ${selectedOption ? (isCorrect ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700') : 'bg-white border-amber-400 text-slate-300'}`}>{selectedOption || "?"}</div><span>{round.after}</span></div></div>
        <div className="p-6 grid grid-cols-4 gap-3 bg-white">{round.options.map((option, idx) => ( <button key={idx} disabled={!!selectedOption} onClick={() => handleOptionClick(option)} className={`aspect-square rounded-xl border-2 text-2xl font-arabic flex items-center justify-center ${selectedOption ? (option === round.correctOption ? "bg-green-100 border-green-500 text-green-800" : option === selectedOption ? "bg-red-100 border-red-500" : "opacity-40") : "hover:bg-amber-50"}`}>{option}</button> ))}</div>
      </div>
    </div>
  );
}
...
function QuizGame({ onBack, onClose, cards, showDiacritics }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/90 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col relative">
        <div className="bg-indigo-600 p-4 text-white flex justify-between items-center"><div className="flex items-center gap-2"><button onClick={onBack} className="hover:bg-indigo-500 p-1 rounded mr-2"><ArrowLeft className="w-5 h-5"/></button><h2 className="font-bold text-lg">Quiz</h2></div><button onClick={onClose}><X className="w-6 h-6" /></button></div>
        <div className="flex justify-between px-6 py-3 bg-indigo-50 border-b border-indigo-100"><span className="font-bold text-indigo-700">Racha: {score}</span><span className="font-bold text-amber-600">Récord: {highScore}</span></div>
        <div className="p-8 text-center bg-slate-50"><h3 className="text-3xl font-black text-slate-800">{currentRound.question.spanish}</h3></div>
        <div className="p-6 grid grid-cols-1 gap-3 bg-white">{currentRound.options.map((option) => ( <button key={option.id} disabled={!!selectedOption} onClick={() => handleOptionClick(option)} className={`p-4 rounded-xl border-2 text-xl font-arabic ${selectedOption ? (option.id === currentRound.question.id ? "bg-green-100 border-green-500" : option.id === selectedOption.id ? "bg-red-100 border-red-500" : "opacity-40") : "hover:bg-indigo-50"}`}>{showDiacritics ? option.arabic : removeArabicDiacritics(option.arabic)}</button> ))}</div>
      </div>
    </div>
  );
}
...
function Flashcard({ data, frontLanguage, showDiacritics, isAdmin, onDelete, onEdit }) {
  ...
  let content = null;
  if (isAdmin) {
    content = <><h3 className="text-lg font-bold text-slate-800 line-clamp-2">{spanishText}</h3><h3 className="text-2xl font-arabic text-emerald-700 mt-1" dir="rtl">{displayArabic}</h3></>;
  } else if (flipState === 2) {
    content = <><p className="text-xs uppercase text-amber-600 font-bold mb-2">Fonética</p><h3 className="text-lg font-mono text-amber-800 italic">{data?.phonetic || "N/A"}</h3></>;
  } else {
    const isFront = flipState === 0;
    const currentLang = isFront ? frontLanguage : (frontLanguage === 'spanish' ? 'arabic' : 'spanish');
    if (currentLang === 'spanish') { content = <><p className="text-xs uppercase text-slate-400 font-bold mb-2">Español</p><h3 className="text-xl font-bold text-slate-800">{spanishText}</h3></>; } 
    else { content = <><p className="text-xs uppercase text-emerald-600 font-bold mb-2">Árabe</p><h3 className="text-3xl font-arabic text-emerald-900 mb-4" dir="rtl">{displayArabic}</h3><button onClick={playAudio} className="p-2 bg-emerald-200 text-emerald-800 rounded-full"><Volume2 className="w-4 h-4"/></button></>; }
  }
  return (
    <div onClick={handleNextFace} className={`relative h-60 w-full rounded-2xl shadow-sm hover:shadow-lg transition-all border flex flex-col p-4 text-center cursor-pointer bg-white`}>
      {isAdmin && <div className="absolute top-2 right-2 flex gap-2"><button onClick={onEdit}><Edit2 className="w-4 h-4"/></button><button onClick={onDelete}><Trash2 className="w-4 h-4"/></button></div>}
