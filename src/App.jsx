...
import { 
  ...
  Wand2, FileText, Loader2, Settings, AlertTriangle, ArrowRight, Check, 
  Trophy, Frown, PartyPopper, ArrowLeft, PlayCircle, HelpCircle, Grid, 
  Activity, Mic, Edit3, Clock, Image as ImageIcon
} from 'lucide-react';
...
const playSmartAudio = (text) => {
    ...
    const preferredVoice = voices.find(v => 
        v.lang.includes('ar') && 
        (v.name.includes('Google') || v.name.includes('Maged') || v.name.includes('Tariq') || v.name.includes('Premium'))
    );
...
export default function App() {
  ...
  const [showDiacritics, setShowDiacritics] = useState(() => JSON.parse(localStorage.getItem('pref_diacritics')) ?? true);
  ...
  const [isGamesHubOpen, setIsGamesHubOpen] = useState(() => JSON.parse(localStorage.getItem('games_open')) || false); 
...
// --- HUB DE JUEGOS CON PERSISTENCIA ---
function GamesHub({ onClose, cards, showDiacritics }) {
  ...
  useEffect(() => {
    localStorage.setItem('current_game', activeGame);
  }, [activeGame]);
                ...
                <div className="p-8 text-center">
                    ...
                    <div className="flex flex-col gap-3">
                        ...
                        <button onClick={() => startGame(5)} className="p-4 bg-orange-100 text-orange-700 rounded-xl font-bold border-2 border-orange-200 hover:bg-orange-200 flex items-center justify-between group">
                            <span className="flex items-center gap-2"><Timer className="w-5 h-5"/> Normal (5s)</span>
                            <ArrowRight className="w-5 h-5 opacity-0 group-hover:opacity-100 transition"/>
                        </button>
