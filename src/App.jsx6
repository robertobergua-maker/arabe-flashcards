...
export default function App() {
  ...
  const categories = useMemo(() => {
    ...
    // Ordenar Pistas numéricamente (1, 2, 3... 10...)
    pistas.sort((a, b) => {
  ...
  const processText = () => {
      try {
        ...
        const newCards = lines
          ...
          .filter(c => c.spanish && c.arabic);
          
        if (newCards.length === 0) { alert("No se encontraron líneas válidas."); return; }
        onImport(newCards);
